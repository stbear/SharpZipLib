...
namespace ICSharpCode.SharpZipLib.Core
{
	...
	public static class StreamUtils
	{
		...
		internal static void WriteProcToStreamSync(this Stream targetStream, MemoryStream bufferStream, Action<Stream> writeProc)
		{
			bufferStream.SetLength(0);
			writeProc(bufferStream);
			bufferStream.Position = 0;
			bufferStream.CopyTo(targetStream, 81920);
			bufferStream.SetLength(0);
		}
		
		internal static void WriteProcToStreamSync(this Stream targetStream, Action<Stream> writeProc)
		{
			using (var ms = new MemoryStream())
			{
				WriteProcToStreamSync(targetStream, ms, writeProc);
			}
		}
	}
}
