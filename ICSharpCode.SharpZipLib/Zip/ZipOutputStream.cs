...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class ZipOutputStream : DeflaterOutputStream
	{
		...
		public void PutNextEntry(ZipEntry entry)
		{
			...
			if (entry.IsCrypted)
			{
				WriteOutputSync(GetEntryEncryptionHeader(entry));
			}
		}
		...
		private void WriteOutputSync(byte[] bytes) 
		...
		/// The Compression method specified for the entry is unsupported.
		/// </exception>
		public void PutNextEntryAsync(ZipEntry entry, CancellationToken ct = default)
		{
			if (curEntry != null) CloseEntrySync();
			var position = CanPatchEntries ? baseOutputStream_.Position : -1; 
			baseOutputStream_.WriteProcToStreamSync(s =>
			{
		...
		private void FinishCompressionSync()
		{
		...
		public void CloseEntrySync()
		{
			FinishCompressionSync();
			baseOutputStream_.WriteProcToStreamSync(WriteEntryFooter);
